...
import SocialButtons from '../../LoginPage/components/SocialButtons';

const validate = {
  name: (v) => {
    if (!v) return '이름을 입력해주세요';
    if (/[^가-힣a-zA-Z0-9\s]/.test(v)) return '이름에 특수문자를 사용할 수 없어요';
    return '';
  },
  email: (v) => {
...
function SignupForm({ theme = 'blue', onLoginClick }) {
  const [form, setForm] = useState({
    name: '', email: '', password: '', passwordConfirm: '', agreed: false,
  });
  ...
  const handleChange = (field) => (e) => {
    ...
    if (touched[field]) {
      const err = field === 'passwordConfirm'
  ...
  const handleSubmit = () => {
    ...
    console.log('회원가입 요청:', { name: form.name, email: form.email });
  };

  // 이름: 빨간 테두리만 / 이메일·비밀번호·비밀번호확인: 테두리 + 에러 메시지
  const inputClass = (field) =>
    errors[field] && touched[field] ? 'login-input input--error' : 'login-input';
  ...
  return (
    <div className={`login-form signup-form--${theme}`}>
      ...
      <p className="signup-error-slot">{showMsg('email') ? errors.email : ''}</p>
      ...
      <p className="signup-error-slot">{showMsg('password') ? errors.password : ''}</p>

      {/* 비밀번호 확인: 테두리 + 에러 메시지 */}
      <label className="login-label" htmlFor="signup-password-confirm">비밀번호 확인</label>
      ...
      <p className="signup-error-slot">{showMsg('passwordConfirm') ? errors.passwordConfirm : ''}</p>
      ...
      <button className={`login-submit signup-submit--${theme}`} onClick={handleSubmit}>
        회원가입
      </button>
      ...
      <div className="login-or"><span>소셜 계정으로 간편 가입</span></div>
      <SocialButtons />
    </div>
  );
}
