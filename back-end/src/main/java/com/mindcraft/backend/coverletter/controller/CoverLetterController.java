...
import com.mindcraft.backend.coverletter.service.CoverLetterExportService; // 추가된 부분: export 서비스 사용을 위해 추가
...
//
    ...
    public ResponseEntity<byte[]> exportPdf(
        ...
        long userId = userSecurityDto.getId();
        CoverLetterDetailDto coverLetterDetailDto = coverLetterService.getDetailById(userId, coverLetterId);
        byte[] pdfBytes = coverLetterExportService.toPdf(coverLetterDetailDto);
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, contentDisposition(coverLetterDetailDto.getTitle(), "pdf"))
    ...
    public ResponseEntity<byte[]> exportDocx(
        ...
        long userId = userSecurityDto.getId();
        CoverLetterDetailDto coverLetterDetailDto = coverLetterService.getDetailById(userId, coverLetterId);
        byte[] docxBytes = coverLetterExportService.toDocx(coverLetterDetailDto);

        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, contentDisposition(coverLetterDetailDto.getTitle(), "docx"))
